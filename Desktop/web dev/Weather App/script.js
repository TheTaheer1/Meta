...
async function fetchData(location) {
  const url = `https://api.weatherapi.com/v1/current.json?key=35af7ff606db422880d141328231305&q=${location}&aqi=yes`;
  ...
  try {
    ...
    updateData(temp, locationName, feelsLike);

  } catch (error) {
    console.log(error);
    alert('Error: Could not fetch weather data. Please check the city name and try again.');
  }
}
